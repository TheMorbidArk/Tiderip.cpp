...
ObjMap::ObjMap(VM *vm)
{
    this->objHeader = new headerObj(vm, ObjType::OT_MAP, vm->mapClass);
    this->capacity = this->count = 0;
    this->entries = nullptr;
}

ObjMap *ObjMap::newObjMap(VM *vm)
{
    ObjMap *objMap = ALLOCATE(vm, ObjMap);
    objMap->objHeader = new headerObj(vm, ObjType::OT_MAP, vm->mapClass);
    objMap->capacity = objMap->count = 0;
    objMap->entries = nullptr;
    return objMap;
}
...
//计算对象的哈希码
static uint32_t hashObj(headerObj *objHeader)
{
    switch (objHeader->type)
    {
    case ObjType::OT_CLASS:  //计算class的哈希值
        return ObjString::hashString(((Class *)objHeader)->name->value.start,
            ((Class *)objHeader)->name->value.length);
    
    case ObjType::OT_RANGE:
    { //计算range对象哈希码
        ObjRange *objRange = (ObjRange *)objHeader;
        return hashNum(objRange->from) ^ hashNum(objRange->to);
    }
    case ObjType::OT_STRING:  //对于字符串,直接返回其hashCode
...
static bool addEntry(Entry *entries, uint32_t capacity, Value key, Value value)
{
    ...
    while (true)
    {
        ...
        if (entries[index].key.type == ValueType::VT_UNDEFINED)
        {
...
static void resizeMap(VM *vm, ObjMap *objMap, uint32_t newCapacity)
{
    ...
    {
        newEntries[idx].key = VT_TO_VALUE(ValueType::VT_UNDEFINED);
        newEntries[idx].value = VT_TO_VALUE(ValueType::VT_FALSE);
        idx++;
    }
    ...
    if (objMap->capacity > 0)
    {
        ...
        {
            //该slot有值
            if (entryArr[idx].key.type != ValueType::VT_UNDEFINED)
            {
...
void ObjMap::mapSet(VM *vm, ObjMap *objMap, Value key, Value value)
{
    //当容量利用率达到80%时扩容
...
Value ObjMap::mapGet(ObjMap *objMap, Value key)
{
    Entry *entry = findEntry(objMap, key);
    if (entry == NULL)
    {
        return VT_TO_VALUE(ValueType::VT_UNDEFINED);
    }
    return entry->value;
}

//回收objMap.entries占用的空间
void ObjMap::clearMap(VM *vm, ObjMap *objMap)
{
    DEALLOCATE_ARRAY(vm, objMap->entries, objMap->count);
    objMap->entries = NULL;
    objMap->capacity = objMap->count = 0;
...
Value ObjMap::removeKey(VM *vm, ObjMap *objMap, Value key)
{
    Entry *entry = findEntry(objMap, key);
    
    if (entry == NULL)
    {
        return VT_TO_VALUE(ValueType::VT_NULL);
    }

//设置开放定址的伪删除
    Value value = entry->value;
    entry->key = VT_TO_VALUE(ValueType::VT_UNDEFINED);
    entry->value = VT_TO_VALUE(ValueType::VT_TRUE);   //值为真,伪删除
