...
#include "ObjList.hpp"
ObjList::ObjList(VM *vm, uint32_t elementNum)
{

}

ObjList *ObjList::newObjList(VM *vm, uint32_t elementNum)
{
    //存储list元素的缓冲区
    ...
    objList->objHeader = new headerObj(vm, ObjType::OT_LIST, vm->listClass);
    return objList;
}
...
Value ObjList::removeElement(VM *vm, ObjList *objList, uint32_t index)
{
...
void ObjList::insertElement(VM *vm, ObjList *objList, uint32_t index, Value value)
{
    ...
    //即最后一个元素要后移1个空间
    Buffer<Value>::BufferAdd(vm, &objList->elements, VT_TO_VALUE(ValueType::VT_NULL));
