    ...
    ObjClosure *objClosure, int argNum)
{
    ...
    //准备上cpu
    ObjThread::prepareFrame(objThread, objClosure, objThread->esp - argNum);
}
...
static ObjUpvalue *createOpenUpvalue(VM *vm, ObjThread *objThread, Value *localVarPtr)
{
    ...
    if (objThread->openUpvalues == nullptr)
    {
        objThread->openUpvalues = new ObjUpvalue(vm, localVarPtr);
        return objThread->openUpvalues;
    }
    
    //下面以upvalue.localVarPtr降序组织openUpvalues
    ObjUpvalue *preUpvalue = nullptr;
    ObjUpvalue *upvalue = objThread->openUpvalues;
    ...
    ObjUpvalue *newUpvalue = new ObjUpvalue(vm, localVarPtr);
...
//修正部分指令操作数
static void patchOperand(Class *thisClass, ObjFn *fn)
{
    int ip = 0;
    OpCode opCode;
    while (true)
    {
        ...
        case OPCODE_STORE_THIS_FIELD:
            //修正子类的field数目  参数是1字节
            fn->instrStream.datas[ip++] += thisClass->superClass->fieldNum;
            break;
        ...
        case OPCODE_SUPER16:
        {
            ...
            fn->constants.datas[superClassIdx] = OBJ_TO_VALUE(thisClass->superClass);
        ...
        case OPCODE_CREATE_CLOSURE:
        {
            ...
            patchOperand(thisClass, VALUE_TO_OBJFN(fn->constants.datas[fnIdx]));
...
static void bindMethodAndPatch(VM *vm, OpCode opCode,
    uint32_t methodIndex, Class *thisClass, Value methodValue)
{
    
    //如果是静态方法,就将类指向meta类(使接收者为meta类)
    if (opCode == OPCODE_STATIC_METHOD)
    {
        thisClass = thisClass->objHeader->thisClass;
    }
    
    Method method;
    method.type = MethodType::MT_SCRIPT;
    method.obj = VALUE_TO_OBJCLOSURE(methodValue);
    
    //修正操作数
    patchOperand(thisClass, method.obj->fn);
    
    //修正过后,绑定method到class
    bindMethod(vm, thisClass, methodIndex, method);
}
...
DECODE
    ...
    CASE(PUSH_NULL):
        PUSH(VT_TO_VALUE(ValueType::VT_NULL));
        LOOP();
    
    CASE(PUSH_FALSE):
        PUSH(VT_TO_VALUE(ValueType::VT_FALSE));
        LOOP();
    
    CASE(PUSH_TRUE):
        PUSH(VT_TO_VALUE(ValueType::VT_TRUE));
        LOOP();
    ...
    CASE(LOAD_CONSTANT):
        ...
        CASE(CALL16):
            ...
            //获得方法所在的类
            thisClass = Class::getClassOfObj(vm, args[0]);
            goto invokeMethod;
        ...
        CASE(SUPER16):
            ...
            thisClass = VALUE_TO_CLASS(fn->constants.datas[READ_SHORT()]);
        
        invokeMethod:
            if ((uint32_t)index > thisClass->methods.count ||
                (method = &thisClass->methods.datas[index])->type == MethodType::MT_NONE)
            {
            ...
            case MethodType::MT_PRIMITIVE:
                ...
                else
                {
                    ...
                    if (!VALUE_IS_NULL(curThread->errorObj))
                    {
                        ...
                        PEEK() = VT_TO_VALUE(ValueType::VT_NULL);
                    }
...
//                NOT_REACHED()
    ...
    CASE(CONSTRUCT):
    {
        ...
        ObjInstance *objInstance = new ObjInstance(vm, VALUE_TO_CLASS(stackStart[0]));
    ...
    CASE(CREATE_CLOSURE):
    {
        ...
        ObjClosure *objClosure = new ObjClosure(vm, objFn);
        //将创建好的闭包的value结构压到栈顶,
    ...
    CASE(CREATE_CLASS):
    {
        ...
        Class *thisClass = new Class(vm, VALUE_TO_OBJSTR(className),
            fieldNum, VALUE_TO_CLASS(superClass));
        
        //类存储于栈底
        stackStart[0] = OBJ_TO_VALUE(thisClass);
    ...
    CASE(STATIC_METHOD):
    {
        ...
        //从栈顶中获得待绑定的类
        Class *thisClass = VALUE_TO_CLASS(PEEK());
        
        //从次栈顶中获得待绑定的方法,
        //这是由OPCODE_CREATE_CLOSURE操作码生成后压到栈中的
        Value method = PEEK2();
        
        bindMethodAndPatch(vm, (OpCode)opCode, methodNameIndex, thisClass, method);
