...
CompileUnit::CompileUnit(Parser *parser, CompileUnit *enclosingUnit, bool isMethod)
{
    parser->curCompileUnit = this;
    this->curParser = parser;
    this->enclosingUnit = enclosingUnit;
    this->curLoop = nullptr;
    this->enclosingClassBK = nullptr;
    
    //若没有外层,说明当前属于模块作用域
    if (enclosingUnit == nullptr)
    {
    ...
    {   //若是内层单元,属局部作用域
        ...
        {  //若是类中的方法
            ...
            this->localVars[0].name = "this";
            this->localVars[0].length = 4;
            
        }
        else
        {      //若为普通函数
            //空出第0个局部变量,保持统一
            this->localVars[0].name = nullptr;
            this->localVars[0].length = 0;
        }
        
        //第0个局部变量的特殊性使其作用域为模块级别
        this->localVars[0].scopeDepth = -1;
        this->localVars[0].isUpvalue = false;
        this->localVarNum = 1;  //localVars[0]被分配
    ...
    this->stackSlotNum = this->localVarNum;
    
    this->fn = new ObjFn(this->curParser->vm, this->curParser->curModule, this->localVarNum);
}

void CompileUnit::initCompileUnit(Parser *parser, CompileUnit *cu, CompileUnit *enclosingUnit, bool isMethod)
{
    parser->curCompileUnit = cu;
    cu->curParser = parser;
    cu->enclosingUnit = enclosingUnit;
    cu->curLoop = nullptr;
    cu->enclosingClassBK = nullptr;
    ...
    cu->fn = new ObjFn(cu->curParser->vm, cu->curParser->curModule, cu->localVarNum);
    
}

//往函数的指令流中写入1字节,返回其索引
static int opCode::writeByte(CompileUnit *cu, int byte)
{
    //若在调试状态,额外在debug->lineNo中写入当前token行号
#if DEBUG
    Buffer<Int>::BufferAdd(cu->curParser->vm,
...
//写入操作码
static void opCode::writeOpCode(CompileUnit *cu, OpCode opCode)
{
    writeByte(cu, opCode);
    //累计需要的运行时空间大小
...
//写入1个字节的操作数
static int opCode::writeByteOperand(CompileUnit *cu, int operand)
{
    return writeByte(cu, operand);
}

//写入2个字节的操作数 按大端字节序写入参数,低地址写高位,高地址写低位
inline static void opCode::writeShortOperand(CompileUnit *cu, int operand)
{
...
//写入操作数为1字节大小的指令
static int opCode::writeOpCodeByteOperand(CompileUnit *cu, OpCode opCode, int operand)
{
...
//写入操作数为2字节大小的指令
static void opCode::writeOpCodeShortOperand(CompileUnit *cu, OpCode opCode, int operand)
{
    writeOpCode(cu, opCode);
...
static int declareVariable(CompileUnit *cu, const char *name, uint32_t length)
{
    ...
    if (cu->scopeDepth == -1)
    {
        int index = defineModuleVar(cu->curParser->vm,
            cu->curParser->curModule, name, length, VT_TO_VALUE(ValueType::VT_NULL));
        if (index == -1)
        {   //重复定义则报错
            char id[MAX_ID_LEN] = { '\0' };
            memcpy(id, name, length);
...
{
    Buffer<Value>::BufferAdd(cu->curParser->vm, &cu->fn->constants, constant);
...
static void emitLoadConstant(CompileUnit *cu, Value value)
{
    ...
    opCode::writeOpCodeShortOperand(cu, OPCODE_LOAD_CONSTANT, index);
}
...
static void emitCallBySignature(CompileUnit *cu, Signature *sign, OpCode opcode)
{
    ...
    opCode::writeOpCodeShortOperand(cu, (OpCode)(opcode + sign->argNum), symbolIndex);
    
    //此时在常量表中预创建一个空slot占位,将来绑定方法时再装入基类
    if (opcode == OPCODE_SUPER0)
    {
        opCode::writeShortOperand(cu, (int)(addConstant(cu, VT_TO_VALUE(ValueType::VT_NULL))));
    }
}
...
static uint32_t discardLocalVar(CompileUnit *cu, int scopeDepth)
{
    ...
    while (localIdx >= 0 && cu->localVars[localIdx].scopeDepth >= scopeDepth)
    {
        if (cu->localVars[localIdx].isUpvalue)
        {
            ...
            opCode::writeByte(cu, OPCODE_CLOSE_UPVALUE);
        }
        else
        {
            //否则就弹出该变量回收空间
            opCode::writeByte(cu, OPCODE_POP);
        }
        localIdx--;
    }
...
static CompileUnit *getEnclosingClassBKUnit(CompileUnit *cu)
{
    while (cu != nullptr)
    {
        if (cu->enclosingClassBK != nullptr)
        {
            return cu;
        }
        cu = cu->enclosingUnit;
...
static void processArgList(CompileUnit *cu, Signature *sign)
{
    ...
    } while (parser::matchToken(cu->curParser, TokenType::TOKEN_COMMA));
}
...
static void processParaList(CompileUnit *cu, Signature *sign)
{
    ...
    do
    {
        ...
        parser::consumeCurToken(cu->curParser, TokenType::TOKEN_ID, "expect variable name!");
...
{
    if (!parser::matchToken(cu->curParser, TokenType::TOKEN_ASSIGN))
    {
        return false;
    }
    ...
    //读取等号右边的形参左边的'('
    parser::consumeCurToken(cu->curParser, TokenType::TOKEN_LEFT_PAREN, "expect '(' after '='!");
    
    //读取形参
    parser::consumeCurToken(cu->curParser, TokenType::TOKEN_ID, "expect ID!");
    //声明形参
    declareVariable(cu, cu->curParser->preToken.start, cu->curParser->preToken.length);
    
    //读取等号右边的形参右边的'('
    parser::consumeCurToken(cu->curParser, TokenType::TOKEN_RIGHT_PAREN, "expect ')' after argument list!");
    sign->argNum++;
    return true;
}
...
static void idMethodSignature(CompileUnit *cu, Signature *sign)
{
    ...
    if (sign->length == 3 && memcmp(sign->name, "new", 3) == 0)
    {
        ...
        if (!parser::matchToken(cu->curParser, TokenType::TOKEN_LEFT_PAREN))
        {
        ...
        //无参数就直接返回
        if (parser::matchToken(cu->curParser, TokenType::TOKEN_RIGHT_PAREN))
        {
            return;
        }
    ...
    parser::consumeCurToken(cu->curParser, TokenType::TOKEN_RIGHT_PAREN, "expect ')' after parameter list!");
}
...
static void infixMethodSignature(CompileUnit *cu, Signature *sign)
{
    ...
    // 中缀运算符只有一个参数,故初始为1
    sign->argNum = 1;
    parser::consumeCurToken(cu->curParser, TokenType::TOKEN_LEFT_PAREN, "expect '(' after infix operator!");
    parser::consumeCurToken(cu->curParser, TokenType::TOKEN_ID, "expect variable name!");
    declareVariable(cu, cu->curParser->preToken.start, cu->curParser->preToken.length);
    parser::consumeCurToken(cu->curParser, TokenType::TOKEN_RIGHT_PAREN, "expect ')' after parameter!");
}
...
static void defineVariable(CompileUnit *cu, uint32_t index)
{
    ...
    if (cu->scopeDepth == -1)
    {
        ...
        opCode::writeOpCodeShortOperand(cu, OPCODE_STORE_MODULE_VAR, index);
        opCode::writeOpCode(cu, OPCODE_POP);  //弹出栈顶数据,因为上面OPCODE_STORE_MODULE_VAR已经将其存储了
    }
}
...
static void emitLoadVariable(CompileUnit *cu, Variable var)
{
    ...
    case VAR_SCOPE_LOCAL:
        //生成加载局部变量入栈的指令
        opCode::writeOpCodeByteOperand(cu, OPCODE_LOAD_LOCAL_VAR, var.index);
        break;
    case VAR_SCOPE_UPVALUE:
        //生成加载upvalue到栈的指令
        opCode::writeOpCodeByteOperand(cu, OPCODE_LOAD_UPVALUE, var.index);
        break;
    case VAR_SCOPE_MODULE:
        //生成加载模块变量到栈的指令
        opCode::writeOpCodeShortOperand(cu, OPCODE_LOAD_MODULE_VAR, var.index);
        break;
    default:
        NOT_REACHED();
    }
...
static void emitStoreVariable(CompileUnit *cu, Variable var)
{
    ...
    case VAR_SCOPE_LOCAL:
        //生成存储局部变量的指令
        opCode::writeOpCodeByteOperand(cu, OPCODE_STORE_LOCAL_VAR, var.index);
        break;
    case VAR_SCOPE_UPVALUE:
        //生成存储upvalue的指令
        opCode::writeOpCodeByteOperand(cu, OPCODE_STORE_UPVALUE, var.index);
        break;
    case VAR_SCOPE_MODULE:
        //生成存储模块变量的指令
        opCode::writeOpCodeShortOperand(cu, OPCODE_STORE_MODULE_VAR, var.index);
        break;
    default:
        NOT_REACHED();
    }
...
{
    if (canAssign && parser::matchToken(cu->curParser, TokenType::TOKEN_ASSIGN))
    {
...
{
    //进入本函数前已经读入了'{'
    while (!parser::matchToken(cu->curParser, TokenType::TOKEN_RIGHT_BRACE))
    {
        if (PEEK_TOKEN(cu->curParser) == TokenType::TOKEN_EOF)
        {
...
static void compileBody(CompileUnit *cu, bool isConstruct)
{
    ...
    if (isConstruct)
    {
        ...
        opCode::writeOpCodeByteOperand(cu, OPCODE_LOAD_LOCAL_VAR, 0);
    }
    else
    {
        //否则加载null占位
        opCode::writeOpCode(cu, OPCODE_PUSH_NULL);
    }
    
    //返回编译结果,若是构造函数就返回this,否则返回null
    opCode::writeOpCode(cu, OPCODE_RETURN);
}
...
#endif
    //标识单元编译结束
    opCode::writeOpCode(cu, OPCODE_END);
    if (cu->enclosingUnit != nullptr)
    {
        ...
        opCode::writeOpCodeShortOperand(cu->enclosingUnit, OPCODE_CREATE_CLOSURE, index);
        ...
        {
            opCode::writeByte(cu->enclosingUnit,
...
static void emitGetterMethodCall(CompileUnit *cu, Signature *sign, OpCode opCode)
{
    ...
    //对method来说可能还传入了块参数
    if (parser::matchToken(cu->curParser, TokenType::TOKEN_LEFT_BRACE))
    {
        newSign.argNum++;
        //进入本if块时,上面的if块未必执行过,
        //此时newSign.type也许还是GETTER,下面要将其设置为METHOD
        newSign.type = SIGN_METHOD;
        CompileUnit fnCU = CompileUnit(cu->curParser, cu, false);
        
        Signature tmpFnSign = { SIGN_METHOD, "", 0, 0 }; //临时用于编译函数
        if (parser::matchToken(cu->curParser, TokenType::TOKEN_BIT_OR))
        {  //若块参数也有参数
            processParaList(&fnCU, &tmpFnSign);    //将形参声明为函数的局部变量
            parser::consumeCurToken(cu->curParser, TokenType::TOKEN_BIT_OR, "expect '|' after argument list!");
        }
...
{
    opCode::writeOpCode(cu, opCode);
    opCode::writeByte(cu, 0xff);       //先写入高位的0xff
    
    //再写入低位的0xff后,减1返回高位地址,此地址将来用于回填.
    return opCode::writeByte(cu, 0xff) - 1;
}
...
static void condition(CompileUnit *cu, bool canAssign UNUSED)
{
    ...
    parser::consumeCurToken(cu->curParser, TokenType::TOKEN_COLON, "expect ':' after true branch!");
    ...
    uint32_t length, OpCode opCode, bool canAssign)
{
    ...
    if (parser::matchToken(cu->curParser, TokenType::TOKEN_ASSIGN) && canAssign)
    {
        sign.type = SIGN_SETTER;
...
{
    if (getEnclosingClassBK(cu) == nullptr)
    {
...
//判断形式super.methodname()
    if (parser::matchToken(cu->curParser, TokenType::TOKEN_DOT))
    {
        parser::consumeCurToken(cu->curParser, TokenType::TOKEN_ID, "expect name after '.'!");
...
static void emitCall(CompileUnit *cu, int numArgs, const char *name, int length)
{
    ...
    opCode::writeOpCodeShortOperand(cu, (OpCode)(OPCODE_CALL0 + numArgs), symbolIndex
    );
}
...
static void emitLoadModuleVar(CompileUnit *cu, const char *name)
{
    ...
    opCode::writeOpCodeShortOperand(cu, OPCODE_LOAD_MODULE_VAR, index
    );
}
...
static void emitStoreModuleVar(CompileUnit *cu, int index)
{
    ...
    opCode::writeOpCodeShortOperand(cu, OPCODE_STORE_MODULE_VAR, index);
    opCode::writeOpCode(cu, OPCODE_POP);  //弹出栈顶数据
}
...
static void callEntry(CompileUnit *cu, bool canAssign)
{
    ...
    parser::consumeCurToken(cu->curParser,
        TokenType::TOKEN_ID, "expect method name after '.'!");
...
static void mapLiteral(CompileUnit *cu, bool canAssign UNUSED)
{
    ...
    {
        //可以创建空map
        if (PEEK_TOKEN(cu->curParser) == TokenType::TOKEN_RIGHT_BRACE)
        {
            break;
        }
        ...
        //读入key后面的冒号
        parser::consumeCurToken(cu->curParser,
            TokenType::TOKEN_COLON, "expect ':' after key!");
    ...
    parser::consumeCurToken(cu->curParser, TokenType::TOKEN_RIGHT_BRACE, "map literal should end with \'}\'!");
}
...
static void parentheses(CompileUnit *cu, bool canAssign UNUSED)
{
    ...
    expression(cu, BP_LOWEST);
    parser::consumeCurToken(cu->curParser, TokenType::TOKEN_RIGHT_PAREN, "expect ')' after expression!");
}
...
//进入本函数后,curToken是'['右面的符号
    ...
    {
        //支持字面量形式定义的空列表
        if (PEEK_TOKEN(cu->curParser) == TokenType::TOKEN_RIGHT_BRACKET)
        {
            break;
        }
        expression(cu, BP_LOWEST);
    ...
    parser::consumeCurToken(cu->curParser, TokenType::TOKEN_RIGHT_BRACKET, "expect ']' after list element!");
}
...
static void subscript(CompileUnit *cu, bool canAssign)
{
    ...
    //确保[]之间不空
    if (parser::matchToken(cu->curParser, TokenType::TOKEN_RIGHT_BRACKET))
    {
    ...
    parser::consumeCurToken(cu->curParser, TokenType::TOKEN_RIGHT_BRACKET, "expect ']' after argument list!");
...
static void subscriptMethodSignature(CompileUnit *cu, Signature *sign)
{
    ...
    sign->length = 0;
    processParaList(cu, sign);
    parser::consumeCurToken(cu->curParser, TokenType::TOKEN_RIGHT_BRACKET, "expect ']' after index list!");
...
//将结果添加到list
        ...
        parser::matchToken(cu->curParser,
            TokenType::TOKEN_INTERPOLATION)); // 处理下一个内嵌表达式,如"a %(b+c) d %(e) f "中的" d %(e)"

//读取最后的字符串,"a %(b+c) d %(e) f "中的"f "
    parser::consumeCurToken(cu->curParser, TokenType::TOKEN_STRING, "expect string at the end of interpolatation!");
...
//true和false的nud方法
    OpCode opCode = cu->curParser->preToken.type ==
        TokenType::TOKEN_TRUE ? OPCODE_PUSH_TRUE : OPCODE_PUSH_FALSE;
    opCode::writeOpCode(cu, opCode
    );
}
...
//处理函数调用
    if (cu->enclosingUnit == nullptr && parser::matchToken(cu->curParser, TokenType::TOKEN_LEFT_PAREN))
    {
        char id[MAX_ID_LEN] = { '\0' };
...
//如果当前正在编译类方法,则直接在该实例对象中加载field
                if (cu->enclosingUnit != nullptr)
                {
                    opCode::writeOpCodeByteOperand(cu,
                        isRead ? OPCODE_LOAD_THIS_FIELD : OPCODE_STORE_THIS_FIELD,
                        fieldIndex);
                }
                else
                {
                    emitLoadThis(cu);
                    opCode::writeOpCodeByteOperand(cu, isRead ? OPCODE_LOAD_FIELD : OPCODE_STORE_FIELD, fieldIndex);
                }
                return;
            }
        }
...
SymbolBindRule Rules[] = {
    ...
    /* TOKEN_ID */                { nullptr, BP_NONE, id, nullptr, idMethodSignature },
    ...
    /* TOKEN_LEFT_BRACKET */      { nullptr, BP_CALL, listLiteral, subscript, subscriptMethodSignature },
...
{
    SymbolBindRule *rule = &Rules[(int)(cu->curParser->preToken.type)];
...
//生成1个参数的签名
    Signature sign = { SIGN_METHOD, rule->id, (uint32_t)(strlen(rule->id)), 1 };
    emitCallBySignature(cu, &sign, OPCODE_CALL0
    );
}
...
//进入expression时,curToken是操作数w, preToken是运算符S
    DenotationFn nud = Rules[(int)(cu->curParser->curToken.type)].nud;

//表达式开头的要么是操作数要么是前缀运算符,必然有nud方法
    ASSERT(nud != NULL, "nud is NULL!");
    
    parser::getNextToken(cu->curParser);  //执行后curToken为运算符T
    
    bool canAssign = rbp < BP_ASSIGN;
    nud(cu, canAssign);   //计算操作数w的值
    
    while (rbp < Rules[(int)(cu->curParser->curToken.type)].lbp)
    {
        DenotationFn led = Rules[(int)(cu->curParser->curToken.type)].led;
        parser::getNextToken(cu->curParser);  //执行后curToken为操作数e
...
{
    parser::consumeCurToken(cu->curParser, TokenType::TOKEN_ID, "missing variable name!");
    Token name = cu->curParser->preToken;
    //只支持定义单个变量
    if (cu->curParser->curToken.type == TokenType::TOKEN_COMMA)
    {
        COMPILE_ERROR(cu->curParser, "'var' only support declaring a variable.");
    }
    
    //一 先判断是否是类中的域定义   确保cu是模块cu
    if (cu->enclosingUnit == nullptr && cu->enclosingClassBK != nullptr)
    {
        if (isStatic)
        {   //静态域
            char *staticFieldId = ALLOCATE_ARRAY(cu->curParser->vm, char, MAX_ID_LEN);
...
{
    parser::consumeCurToken(cu->curParser, TokenType::TOKEN_LEFT_PAREN, "missing '(' after if!");
    expression(cu, BP_LOWEST);   //生成计算if条件表达式的指令步骤
    parser::consumeCurToken(cu->curParser,
        TokenType::TOKEN_RIGHT_PAREN, "missing ')' before '{' in if!");
...
Label_IF:
    if (parser::matchToken(cu->curParser, TokenType::TOKEN_ELIF))
    {
        uint32_t falseBranchEnd = emitInstrWithPlaceholder(cu, OPCODE_JUMP);
    ...
    else if (parser::matchToken(cu->curParser, TokenType::TOKEN_ELSE))
    {  //如果有else分支
        //添加跳过else分支的跳转指令
        uint32_t falseBranchEnd = emitInstrWithPlaceholder(cu, OPCODE_JUMP);
...
static void leaveLoopPatch(CompileUnit *cu)
{
    ...
    opCode::writeOpCodeShortOperand(cu, OPCODE_LOOP, loopBackOffset);
...
static void compileWhileStatment(CompileUnit *cu)
{
    ...
    parser::consumeCurToken(cu->curParser,
        TokenType::TOKEN_LEFT_PAREN, "expect '(' befor conditino!");
    //生成计算条件表达式的指令步骤,结果在栈顶
    expression(cu, BP_LOWEST);
    parser::consumeCurToken(cu->curParser,
        TokenType::TOKEN_RIGHT_PAREN, "expect ')' after condition!");
...
//   }
    ...
    parser::consumeCurToken(cu->curParser,
        TokenType::TOKEN_ID, "expect variable after for!");
    const char *loopVarName = cu->curParser->preToken.start;
    uint32_t loopVarLen = cu->curParser->preToken.length;
    
    parser::consumeCurToken(cu->curParser,
        TokenType::TOKEN_LEFT_PAREN, "expect '(' befor sequence!");
    
    //编译迭代序列
    expression(cu, BP_LOWEST);
    parser::consumeCurToken(cu->curParser, TokenType::TOKEN_RIGHT_PAREN, "expect ')' after sequence!");
    //申请一局部变量"seq "来存储序列对象,
    ...
    opCode::writeOpCodeByteOperand(cu, OPCODE_LOAD_LOCAL_VAR, seqSlot);
    //2 再压入参数iter,即"seq.iterate(iter)"中的iter
    opCode::writeOpCodeByteOperand(cu, OPCODE_LOAD_LOCAL_VAR, iterSlot);
    //3 调用"seq.iterate(iter)"
    emitCall(cu, 1, "iterate(_)", 10);
    
    //"seq.iterate(iter)"把结果(下一个迭代器)存储到
    //args[0](即栈顶),现在将其同步到变量iter
    opCode::writeOpCodeByteOperand(cu, OPCODE_STORE_LOCAL_VAR, iterSlot);
    
    //如果条件失败则跳出循环体,目前不知道循环体的结束地址,
    //先写入占位符.
    loop.exitIndex = (int)emitInstrWithPlaceholder(cu, OPCODE_JUMP_IF_FALSE);
...
inline static void compileReturn(CompileUnit *cu)
{
    ...
    opCode::writeOpCode(cu, OPCODE_RETURN);   //将上面栈顶的值返回
}
...
{
    if (parser::matchToken(cu->curParser, TokenType::TOKEN_IF))
    {
        compileIfStatment(cu);
    }
    else if (parser::matchToken(cu->curParser, TokenType::TOKEN_WHILE))
    {
        compileWhileStatment(cu);
    }
    else if (parser::matchToken(cu->curParser, TokenType::TOKEN_FOR))
    {
        compileForStatment(cu);
    }
    else if (parser::matchToken(cu->curParser, TokenType::TOKEN_RETURN))
    {
        compileReturn(cu);
    }
    else if (parser::matchToken(cu->curParser, TokenType::TOKEN_BREAK))
    {
        compileBreak(cu);
    }
    else if (parser::matchToken(cu->curParser, TokenType::TOKEN_CONTINUE))
    {
        compileContinue(cu);
    }
    else if (parser::matchToken(cu->curParser, TokenType::TOKEN_LEFT_BRACE))
    {
        //代码块有单独的作用域
        enterScope(cu);
        compileBlock(cu);
    ...
    else
    {
        ...
        //表达式的结果不重要,弹出栈顶结果
        opCode::writeOpCode(cu, OPCODE_POP);
    }
}
...
static int declareMethod(CompileUnit *cu, char *signStr, uint32_t length)
{
    ...
    //下面判断方法是否已定义 即排除重定义
    Buffer<Int> *methods = cu->enclosingClassBK->inStatic ?
    ...
    Buffer<Int>::BufferAdd(cu->curParser->vm, methods, index);
    return index;
}
...
//3 生成OPCODE_STATIC_METHOD或OPCODE_INSTANCE_METHOD
    ...
    opCode::writeOpCodeShortOperand(cu, opCode, methodIndex);
}
...
{
    CompileUnit methodCU(cu->curParser, cu, true);

//1 生成OPCODE_CONSTRUCT指令,该指令生成新实例存储到stack[0].
    opCode::writeOpCode(&methodCU, OPCODE_CONSTRUCT);

//2 生成OPCODE_CALLx指令,该指令调用新实例的构造函数.
    opCode::writeOpCodeShortOperand(&methodCU,
        (OpCode)(OPCODE_CALL0 + sign->argNum), (int)constructorIndex);

//生成return指令,将栈顶中的实例返回
    opCode::writeOpCode(&methodCU, OPCODE_RETURN);
...
static void compileMethod(CompileUnit *cu, Variable classVar, bool isStatic)
{
    ...
    methodSignatureFn methodSign =
        Rules[(int)(cu->curParser->curToken.type)].methodSign;
    ...
    parser::getNextToken(cu->curParser);
    ...
    parser::consumeCurToken(cu->curParser, TokenType::TOKEN_LEFT_BRACE,
...
static void compileClassBody(CompileUnit *cu, Variable classVar)
{
    ...
    if (parser::matchToken(cu->curParser, TokenType::TOKEN_STATIC))
    {
        if (parser::matchToken(cu->curParser, TokenType::TOKEN_TIDE))
        {  //处理静态域 "static var id"
...
static void compileClassDefinition(CompileUnit *cu)
{
    ...
    parser::consumeCurToken(cu->curParser, TokenType::TOKEN_ID,
    ...
    //生成类名,用于创建类
    ObjString *className = new ObjString(cu->curParser->vm,
        cu->curParser->preToken.start, cu->curParser->preToken.length);
    
    //生成加载类名的指令
    emitLoadConstant(cu, OBJ_TO_VALUE(className));
    if (parser::matchToken(cu->curParser, TokenType::TOKEN_LESS))
    {   //类继承
    ...
    int fieldNumIndex = opCode::writeOpCodeByteOperand(cu, OPCODE_CREATE_CLASS, 255);
    ...
    Buffer<String>::BufferInit(&classBK.fields);
    Buffer<Int>::BufferInit(&classBK.instantMethods);
    Buffer<Int>::BufferInit(&classBK.staticMethods);
    ...
    Buffer<Int>::BufferClear(cu->curParser->vm, &classBK.instantMethods);
    Buffer<Int>::BufferClear(cu->curParser->vm, &classBK.staticMethods);
...
static void compileFunctionDefinition(CompileUnit *cu)
{
    ...
    parser::consumeCurToken(cu->curParser, TokenType::TOKEN_ID, "missing function name!");
    ...
    //生成fnCU,专用于存储函数指令流
    CompileUnit fnCU(cu->curParser, cu, false);
    
    Signature tmpFnSign = { SIGN_METHOD, "", 0, 0 }; //临时用于编译函数
    parser::consumeCurToken(cu->curParser,
        TokenType::TOKEN_LEFT_PAREN, "expect '(' after function name!");
    ...
    if (!parser::matchToken(cu->curParser, TokenType::TOKEN_RIGHT_PAREN))
    {
        processParaList(&fnCU, &tmpFnSign);    //将形参声明为函数的局部变量
        parser::consumeCurToken(cu->curParser,
            TokenType::TOKEN_RIGHT_PAREN, "expect ')' after parameter list!");
    }
    
    fnCU.fn->argNum = tmpFnSign.argNum;
    
    parser::consumeCurToken(cu->curParser,
        TokenType::TOKEN_LEFT_BRACE, "expect '{' at the beginning of method body.");
...
//  var bar2 = System.getModuleVariable("foo", "bar2")
    ...
    parser::consumeCurToken(cu->curParser, TokenType::TOKEN_ID, "expect module name after export!");
    ...
    if (cu->curParser->preToken.start[cu->curParser->preToken.length] == '.')
    {
        ...
        //跳过扩展名
        parser::getNextToken(cu->curParser); //跳过'.'
        parser::getNextToken(cu->curParser); //跳过"sp"
    }
    
    //把模块名转为字符串,存储为常量
    ObjString *moduleName = new ObjString(cu->curParser->vm,
    ...
    opCode::writeOpCodeShortOperand(cu, OPCODE_LOAD_CONSTANT, constModIdx);
    ...
    //如果后面没有关键字for就导入结束
    if (!parser::matchToken(cu->curParser, TokenType::TOKEN_FOR))
    {
        return;
    }
    ...
    do
    {
        ...
        //把模块变量转为字符串,存储为常量
        ObjString *constVarName = new ObjString(cu->curParser->vm,
            cu->curParser->preToken.start, cu->curParser->preToken.length);
        ...
        opCode::writeOpCodeShortOperand(cu, OPCODE_LOAD_CONSTANT, constVarIdx);
...
ObjFn *compileModule(VM *vm, ObjModule *objModule, const char *moduleCode)
{
    ...
    if (objModule->name == nullptr)
    {
        // 核心模块是core.script.inc
        fileName = "core.script.include";
    }
    else
    {
        fileName = (const char *)objModule->name->value.start;
    }
    
    //各源码模块文件需要单独的parser
    Parser parser = Parser(vm, fileName, moduleCode, objModule);
    parser.parent = vm->curParser;
    vm->curParser = &parser;
    
    CompileUnit moduleCU = CompileUnit(&parser, nullptr, false);
    ...
    while (!parser::matchToken(&parser, TokenType::TOKEN_EOF))
    {
